...
///     Note that the OpenRead/OpenReadWrite method as well as all operations performed on this stream must be wrapped
...
public sealed class NpgsqlLargeObjectStream : Stream
{
    ...
    /// <summary>
    ///     Returns the current position in the stream. Getting the current position does not need a round-trip to the server,
    ///     however setting the current position does.
    /// </summary>
    ...
    /// <summary>
    ///     Gets the length of the large object. This internally seeks to the end of the stream to retrieve the length, and
    ///     then back again.
    /// </summary>
    ...
    /// <summary>
    ///     Reads <i>count</i> bytes from the large object. The only case when fewer bytes are read is when end of stream is
    ///     reached.
    /// </summary>
    ...
    {
        return Read(false, buffer, offset, count).GetAwaiter().GetResult();
    }
    ...
    {
        return Read(true, buffer, offset, count, cancellationToken);
    }

    private async Task<int> Read(bool async, byte[] buffer, int offset, int count,
        CancellationToken cancellationToken = default)
    {
        ...
        while (read < count)
        {
            ...
            _pos += bytesRead;
            read += bytesRead;
            if (bytesRead < chunkCount) return read;
        }
    ...
    {
        Write(false, buffer, offset, count).GetAwaiter().GetResult();
    }
    ...
    {
        return Write(true, buffer, offset, count, cancellationToken);
    }

    private async Task Write(bool async, byte[] buffer, int offset, int count,
        CancellationToken cancellationToken = default)
    {
        if (buffer == null)
            throw new ArgumentNullException(nameof(buffer));
        ...
        while (totalWritten < count)
        {
            ...
            var bytesWritten = await _manager.ExecuteFunction<int>(async, "lowrite", cancellationToken, _fd,
                new ArraySegment<byte>(buffer, offset + totalWritten, chunkSize)).ConfigureAwait(false);
    ...
    private async Task<long> GetLength(bool async)
    {
        CheckDisposed();
        var old = _pos;
    ...
    {
        return Seek(false, offset, origin).GetAwaiter().GetResult();
    }
    ...
    {
        return Seek(true, offset, origin, cancellationToken);
    }

    private async Task<long> Seek(bool async, long offset, SeekOrigin origin,
        CancellationToken cancellationToken = default)
    {
        ...
        return _manager.Has64BitSupport
            ? _pos = await _manager
                .ExecuteFunction<long>(async, "lo_lseek64", cancellationToken, _fd, offset, (int)origin)
                .ConfigureAwait(false)
            : _pos = await _manager
                .ExecuteFunction<int>(async, "lo_lseek", cancellationToken, _fd, (int)offset, (int)origin)
                .ConfigureAwait(false);
    }
    ...
    /// <summary>
    ///     Truncates or enlarges the large object to the given size. If enlarging, the large object is extended with null
    ///     bytes.
    ...
    {
        SetLength(false, value).GetAwaiter().GetResult();
    }
    ...
    {
        return SetLength(true, value, cancellationToken);
    }

    private async Task SetLength(bool async, long value, CancellationToken cancellationToken = default)
    {
    ...
    public override void Close()
    {
        if (!_disposed)
        {
            _manager.ExecuteFunction<int>(false, "lo_close", CancellationToken.None, _fd).GetAwaiter().GetResult();
            _disposed = true;
        }
    }
