...
public class NpgsqlLargeObjectManager
{
    ...
    /// <summary>
    ///     Creates an NpgsqlLargeObjectManager for this connection. The connection must be opened to perform remote
    ///     operations.
    /// </summary>
    ...
    /// <summary>
    ///     The largest chunk size (in bytes) read and write operations will read/write each roundtrip to the network. Default
    ///     4 MB.
    /// </summary>
    ...
    ///     Execute a function
    /// </summary>
    internal async Task<T> ExecuteFunction<T>(bool async, string function, CancellationToken cancellationToken,
    ...
    internal async Task<int> ExecuteFunctionGetBytes(
        bool async, string function, byte[] buffer, int offset, int len, CancellationToken cancellationToken,
        params object[] arguments)
    {
        using var command = Connection.CreateCommand();
        var stringBuilder = new StringBuilder("SELECT * FROM ").Append(function).Append('(');
    ...
    /// <summary>
    ///     Create an empty large object in the database. If an oid is specified but is already in use, an PostgresException
    ///     will be thrown.
    /// </summary>
    ...
    {
        return Create(preferredOid, false).GetAwaiter().GetResult();
    }
    ...
    {
        return Create(preferredOid, true, cancellationToken);
    }

    private Task<uint> Create(uint preferredOid, bool async, CancellationToken cancellationToken = default)
    {
        return ExecuteFunction<uint>(async, "lo_create", cancellationToken, (int)preferredOid);
    }
    ...
    {
        return OpenRead(false, oid).GetAwaiter().GetResult();
    }
    ...
    {
        return OpenRead(true, oid, cancellationToken);
    }

    private async Task<NpgsqlLargeObjectStream> OpenRead(bool async, uint oid,
    ...
    {
        return OpenReadWrite(false, oid).GetAwaiter().GetResult();
    }
    ...
    {
        return OpenReadWrite(true, oid, cancellationToken);
    }

    private async Task<NpgsqlLargeObjectStream> OpenReadWrite(bool async, uint oid,
        CancellationToken cancellationToken = default)
    {
    ...
    {
        ExecuteFunction<object>(false, "lo_unlink", CancellationToken.None, (int)oid).GetAwaiter().GetResult();
    }
    ...
    {
        return ExecuteFunction<object>(true, "lo_unlink", cancellationToken, (int)oid);
    }
    ...
    {
        ExecuteFunction<object>(false, "lo_export", CancellationToken.None, (int)oid, path).GetAwaiter().GetResult();
    }
    ...
    {
        return ExecuteFunction<object>(true, "lo_export", cancellationToken, (int)oid, path);
    }

    /// <summary>
    ///     Imports a large object to be stored as a large object in the database from a file stored on the backend. This
    ...
    {
        ExecuteFunction<object>(false, "lo_import", CancellationToken.None, path, (int)oid).GetAwaiter().GetResult();
    }
    ...
    {
        return ExecuteFunction<object>(true, "lo_import", cancellationToken, path, (int)oid);
    }
}